...
module rec Load : sig
  ...
  let sites_locations_register_code sctx ~dir (def:Sites_locations.t) =
    let package =
      match Package.Name.Map.find (Super_context.packages sctx) def.package with
      | Some p -> p
      | None ->
        User_error.raise [ Pp.text "sites_locations used outside a package" ]
    in
    let pr buf fmt = Printf.bprintf buf (fmt ^^ "\n") in
    let buf = Buffer.create 1024 in
    (* Parse the replacement format described in [artifact_substitution.ml]. *)
    Package.Name.Map.iteri package.sites_locations
      ~f:(fun name section ->
        ...
        pr buf "  ~suffix:%S" (Package.Name.to_string name);
        pr buf "  ~encoded:%S"
  ...
  let load_text_files sctx ft_dir
    ...
    let generated_files =
      List.concat_map stanzas ~f:(fun stanza ->
          ...
          | Sites_locations def ->
            sites_locations_register_code sctx ~dir def
          | Library { buildable; _ }
